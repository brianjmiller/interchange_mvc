...
{
    my $calculate_dependencies;
    {
        my $dependencies = {};

        $calculate_dependencies = sub {
            my $self = shift;
            my $db   = shift;
            my $data = shift;
            ...
            if (ref $data eq 'HASH' and $data->{__dependency}) {
                ...
                while (my ($key, $val) = each %$data) {
                    $data->{$key} = $calculate_dependencies->($self, $db, $val);
                }
            }
            elsif (ref $data eq 'ARRAY' and @$data) {
                for my $element (@$data) {
                    $element = $calculate_dependencies->($self, $db, $element);
                }
            }
        ...
        sub build_data {
            ...
            if (@$build_keys) {
                ...
                for my $build_key (@$build_keys) {
                    ...
                    $config_data->{object_data} = $calculate_dependencies->($self, $db, $config_data->{object_data});
                ...
                while (my ($key, $ref) = each %$build_data) {
                    ...
                    if ($ref->{config}->{post_hooks}) {
                        ...
                        for my $hook (@{ $ref->{config}->{post_hooks} }) {
                            ...
                            my @method_args = defined $hook->{method_args} ? @{ $calculate_dependencies->($self, $db, $hook->{method_args}) } : ();
